...
It looks like we still have some duplicate policies, where things like "Platform Policy" and "Platform Policies" have come out as distinct. Let's file those down:

```{r}
fb_words %>%
  filter(word_count > 250) %>%
  filter(between(target_dt, as.Date("2008-01-01"), as.Date("2009-09-01"))) %>%
  filter(target_dt == max(target_dt)) %>%
  select(policy_name, word_count, snapshot_url) %>%
  print() %>%
  pull(snapshot_url)
  
  group_by(target_dt) %>%
  distinct(snapshot_url)

```

But it looks like some policies inadvertently scraped 404 pages or bad redirection pages:

```{r}
fb_words %>% filter(word_count < 100)

fb_words %>% filter(word_count < 100) %>% pull(target_url) %>% unique()
```

I'll try to scrape those manually:

```{r}
#| label: manualfbscrape
fb_words_pagesgroups <- analyse_facebook(
  primary_url = "https://www.facebook.com/policies_center/pages_groups_events",
  primary_name = "Pages, Groups, and Events",
  scrape_links = "FALSE")
fb_words_ads <- analyse_facebook(
  primary_url = "https://www.facebook.com/policies/ads/",
  primary_name = "Advertising Policies",
  scrape_links = "FALSE")
```
    ...
    if_else(type == "primary", "Primary terms", "Secondary agreements")) %>%
  {
    ggplot(.) +
      ...
      labs(x = NULL, y = NULL, fill = NULL,
        ...
        subtitle = paste(
          "Since beginning in 2005, Facebook's terms of service have grown from **two policies under",
          "3 000 words** into a behomoth comprising **at least nine policies and nearly 30 000 words.**",
          sep = "<br>"),
...
```{r}
#| label: setupspotify
spotify_css <- css = css_tries = c("#content-main", "#pbody")
...
spotify_words <- analyse_spotify(
  primary_url = "https://www.spotify.com/au/legal/end-user-agreement/")
```