...
# get the 3 primary sets of terms for each month since 2005
fb_primary_terms <- tibble(
  url = c(
    "https://www.facebook.com/legal/terms",
    "https://www.facebook.com/about/privacy/update",
    "https://www.facebook.com/policies/cookies"),
  css = c("#content", "._5tkp", "._5tkp"))
date_seq <- seq(as.Date("2005-12-15"), Sys.Date(), by = "month")

expand_grid(fb_primary_terms, date_seq) %>%
  tibble(
    url = c(
      "https://www.facebook.com/legal/terms",
      "https://www.facebook.com/about/privacy/update",
...
fb_terms_firststage %>%
  ...
  distinct(dt, url, .keep_all = TRUE) %>% select(label, url) %>% print(n = Inf)
  mutate(
    css = "",
    sec_scrape = map2_peacefully(url, css, get_words_and_links),
    sec_words = map(sec_scrape, c("result", "words")),
    sec_links = map(sec_scrape, c("result", "links"))
    )
```