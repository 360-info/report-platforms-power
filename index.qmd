...
Finally, a quick function to save our processed word counts:
...
Here're the CSS selectors that I think capture most of Facebook's redesigns:
...
Although Waybaack Machine does substitute links in snapshotted terms with snapshotted URLs, I've opted not to use them and to re-request a nearby snapshot from the original link URL, as related policies are not always snapshotted at the exact same time.
...
fb_terms_firststage %>%
  mutate(
    label = "Terms of Use",
    word_count = map_int(prim_words, nrow),
    link_count = map_int(prim_links, nrow)) %>%
  # write the processed terms out to csvs
  {
    pwalk(
      select(., terms = prim_words, date = dt, policy = label),
      save_terms, platform = "facebook")
  } %>%
  # now write the stats out
  select(policy = label, req_dt = dt, req_url = url, snapshot_dt = prim_snapshot_dt,
    snapshot_url = prim_snapshot_url, word_count, link_count) %>%
  write_csv(here("data", "facebook-count-primary.csv"))
```
